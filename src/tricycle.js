...
function onCollision(other_object, linear_velocity, angular_velocity ){
	if(other_object.name==="cart"){
       ...
       // var force = THREE.Vector3(0, 0, 1000);
    //    var newForce = ball.matrix.multiplyVector3(force);
        tocando=true;
	}
}
    ...
    switch( keyEvent.keyCode ) {
        ...
        case 90:
		   if(tocando){
			...
			ball.applyCentralImpulse(new THREE.Vector3(0,120,0));
            }
		break;    
	}
}
...
function handleKeyUp(keyEvent){
   switch( keyEvent.keyCode ) {
		...
		break;
		   ...
		   if(tocando){
            tocando=false;
			ball.applyCentralImpulse(new THREE.Vector3(0,0,0));
            }
		break;  
	}
}
